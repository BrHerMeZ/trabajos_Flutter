...
function Login() {
  ...
  const [toast, setToast] = useState({// Estado para controlar la visibilidad del toast
  ...
  const showToast = (message, type = 'success') => { // Función para mostrar el toast
    setToast({// Actualiza el estado del toast para mostrarlo
      isVisible: true,
      message,
      type
    });
  ...
  const handleSubmit = async (e) => {// Función para manejar el envío del formulario
    ...
    } catch (error) { // Manejo de errores al conectar con el servidor
      console.error('Error al conectar:', error);// Imprimir el error en la consola
      const errorMessage = 'No se pudo conectar con el servidor';// Mensaje de error genérico
      setError(errorMessage);// Actualizar el estado de error
      showToast(errorMessage, 'error');// Mostrar el toast de error
    }
  };
